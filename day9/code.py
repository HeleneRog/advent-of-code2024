...
class Block:
    ...
    def can_fill(self, size):
        return (not self.position and self.size <= size)


blocks_map = dict([(int(index/2), Block(e))
                  for index, e in enumerate(line) if (index % 2) == 0])
position = 0
...
for index, size in enumerate(line):
    if (index % 2) == 0:
        ...
        if not blocks_map[file].position:
            res_part2 += blocks_map[file].set_position_and_compute_score(
                position, file)
        position += blocks_map[file].size
    else:
        while size > 0:
            try:
                rev_lst = reversed(list(blocks_map.items()))
                file, block = next((f, b)
                                   for (f, b) in rev_lst if b.can_fill(size))
                res_part2 += blocks_map[file].set_position_and_compute_score(
                    position, file)
                position += block.size
                size -= block.size
